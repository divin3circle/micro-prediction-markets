...
function AgentBanner({ online }) {
  const cls =
    online === null ? "bg-[#1e1e24] text-[#A1A1B0]"
  ...
  const dot =
    ...
    :                 "bg-red-500";
  return (
    <div className={`flex items-center gap-2 rounded-xl px-4 py-2 text-xs ${cls}`}>
...
export function StatsView() {
  ...
  const displayed = useMemo(() => {
    if (tab === "verdicts") return markets.filter((m) => Boolean(verdicts[m.id]));
    ...
    <div className="space-y-6">
      ...
      {loading && !stats ? (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="h-20 animate-pulse rounded-xl bg-[#0D0D0F]" />
          ))}
        </div>
      ) : stats ? (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          ...
          <StatCard label="Total volume"   value={fmt(stats.totalVolume)} sub="INIT" />
          <StatCard label="AI verdicts"    value={Object.keys(verdicts).length} />
        </div>
      ) : (
        <div className="rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-400">
          Could not reach the AI agent backend. Is <code>pulsemarket-agent</code> running on port 3001?
        </div>
      )}
      ...
      {displayed.length === 0 ? (
        <p className="text-center text-sm text-[#A1A1B0] py-8">
          {tab === "verdicts" ? "No AI verdicts yet — they appear after markets close." : "No markets yet."}
        </p>
      ) : (
        <div className="space-y-3">
          {displayed.map((m) => (
            <VerdictCard key={m.id} market={m} verdict={verdicts[m.id] ?? null} />
          ))}
        </div>
      )}
    </div>
